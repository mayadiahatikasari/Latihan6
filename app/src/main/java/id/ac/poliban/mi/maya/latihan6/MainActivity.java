package id.ac.poliban.mi.maya.latihan6;
...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        if (item.getItemId()==R.id.miCherryBarb){
            ...
            //terlihat gambar cherry barb sebagai
            //hasil import android drawble importer
            //dikenali dalam resource kode kita
            view.setBackgroundResource(R.drawable.cherry_barb);
        }

        if (item.getItemId()==R.id.miGuppiess){
            View view = this.getWindow().getDecorView();
